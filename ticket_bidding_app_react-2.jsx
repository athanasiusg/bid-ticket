        ...
        {items.length === 0 ? (
          ...
          <div className="flex flex-col gap-2">
            {items.map((b) => {
              ...
              const ok = !["toolow", "tooslow", "FAILED"].includes(b.status);
              ...
              return (
                ...
                >
                  ...
                  <span className={`shrink-0 text-[10px] font-black uppercase tracking-widest ${tone}`}>
                    {failed ? "Local only" : ok ? "Valid" : "Rejected"}
                  </span>
                </div>
              );
            })}
...
// ── Database (Server-Side Example)
// This section assumes you are using Next.js (App Router or Pages Router)
// and deploying on Vercel. DO NOT run this in the browser.
// DATABASE_URL is read from process.env.DATABASE_URL.

/*
Example: /app/api/init-db/route.ts   (Next.js App Router)
...
const pool = new Pool({
  ...
  ssl: { rejectUnauthorized: false }, // Required for most Vercel-hosted Postgres
});
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Bids table ready" });
  } catch (error) {
    console.error(error);
...
// After deploying, hit /api/init-db once to create the table.

/*
Real bidding API route using process.env.DATABASE_URL

Example: /app/api/bids/route.ts (Next.js App Router)
...
export async function POST(req: Request) {
  ...
  const { event_id, ticket_id, section, amount, status } = body;

  const result = await pool.query(
    `INSERT INTO "Bids"(event_id, ticket_id, section, amount, status)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id, event_id, ticket_id, section, amount, status, created_at`,
    [event_id, ticket_id, section, amount, status]
  );
...
export async function GET(req: Request) {
  ...
  const result = await pool.query(
    `SELECT id, event_id, ticket_id, section, amount, status, created_at
     FROM "Bids"
     WHERE ($1::int IS NULL OR event_id = $1::int)
...
export default function BidTicket() {
  ...
  const [bidSyncStatus, setBidSyncStatus] = useState("idle"); // idle | loading | error | ok

  // Real bidding: persist bids server-side via /api/bids.
  // The API route (server-side) uses process.env.DATABASE_URL.
  const persistBid = async (entry) => {
    if (!selectedEvent) return;

    const tempId = `tmp_${Date.now().toString(16)}_${Math.random().toString(16).slice(2)}`;
    const tempRow = {
      id: tempId,
    ...
    try {
      const res = await fetch("/api/bids", {
        ...
        body: JSON.stringify({
          ...
          amount: entry.amount,
          status: entry.status,
          client_created_at: entry.at,
        }),
      });

      if (!res.ok) throw new Error("Bid save failed");
      const data = await res.json();
      const saved = data?.bid;
      if (!saved?.id) throw new Error("Invalid bid response");
      ...
      // mark temp row as failed
      setBidLog((prev) => prev.map((b) => (b.id === tempId ? { ...b, status: "FAILED" } : b)));
    }
  };

  // When an event is opened, load its bids from the server.
  useEffect(() => {
    const load = async () => {
      if (!selectedEvent) return;
      setBidSyncStatus("loading");
      try {
        const res = await fetch(`/api/bids?event_id=${encodeURIComponent(selectedEvent.id)}`);
        if (!res.ok) throw new Error("Fetch failed");
        const data = await res.json();
        const rows = Array.isArray(data?.bids) ? data.bids : [];
        ...
        setBidLog(normalized);
        setBidSyncStatus("ok");
      } catch {
        setBidSyncStatus("error");
      }
    };
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 py-10">
        ...
        {selectedEvent ? (
          <div>
            ...
            {bidSyncStatus === "loading" ? (
              <div className="mb-6 text-white/40 text-xs">Syncing bids…</div>
            ) : bidSyncStatus === "error" ? (
              <div className="mb-6 text-rose-300 text-xs">Could not load bids from server. Showing local activity only.</div>
            ) : null}
